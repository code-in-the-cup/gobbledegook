...
// SERVER DATA
// -----------------------------------------------------------------------------------------------------------------------------
// Type definition for a delegate that will configure GATT
// It will be called once during server setup
typedef void (*GGKServerConfigurator)(ggk::DBusObject &dBusObject);

// Type definition for a delegate that the server will usDBusObjecte when it needs to receive data from the host application
//
// IMPORTANT:
//
...
int ggkStart(const char *pServiceName, const char *pAdvertisingName, const char *pAdvertisingShortName,
    GGKServerConfigurator configurator, GGKServerDataGetter getter, GGKServerDataSetter setter, int maxAsyncInitTimeoutMS);
